...
if __name__ == '__main__':
    # att_train_datas = data.load_paired_data(converter,normalizer,datapath='/workspace/att_data',filter=None,all_data=True)
    # with open('finetune/train_datas.pkl', 'wb') as f:
    #     pickle.dump(train_datas, f)
    # with open('finetune/first_traindata.pkl', 'wb') as f:
    #     pickle.dump(att_train_datas, f)
    # cr_data = create_data(att_train_datas)
    # att_train_datas = cr_data
    # att_neg_datas = {}
    # for i in tqdm(range(len(att_train_datas))):
    #     for file in att_train_datas[i].keys():
    #         if len(file.split('@')) == 4:
    #             target = '@'.join(file.split('@')[-2:])
    #             if target not in att_neg_datas.keys():
    #                 att_neg_datas[target] = {}
    #             att_neg_datas[target][file] = att_train_datas[i][file]
    #         else:
    #             source = '@'.join(file.split('@')[:2])
    #             if source not in att_neg_datas.keys():
    #                 att_neg_datas[source] = {}
    #             att_neg_datas[source][file] = att_train_datas[i][file]


    # with open('finetune/att_datas.pkl', 'wb') as f:
    #     pickle.dump(att_neg_datas, f)
    
    with open('finetune/att_datas.pkl', 'rb') as f:
        att_neg_datas = pickle.load(f)
    att_neg_train = []
    att_pos_train = []
    att_anchor_train = []
    o_name = ['o1', 'o2', 'o3', 'os', 'of', 'o0']
    for proj in att_neg_datas.keys():
        my_o_name = []
        for opt in o_name:
            if proj+'@'+opt in att_neg_datas[proj].keys():
                my_o_name.append(opt)
        if len(my_o_name)<=1:
            continue
        for filename in att_neg_datas[proj].keys():
            source = '@'.join(filename.split('@')[:2])
            if source != proj:
                rd_sam = random.sample(my_o_name, 2)
                g, feat = att_neg_datas[proj][proj+'@'+rd_sam[0]]
                att_anchor_train.append(S2VGraph(g,feat))
                g, feat = att_neg_datas[proj][filename]
                att_neg_train.append(S2VGraph(g,feat))
                g, feat = att_neg_datas[proj][proj+'@'+rd_sam[1]]
                att_pos_train.append(S2VGraph(g,feat))
    with open('finetune/att_train.pkl', 'wb') as f:
        pickle.dump([att_anchor_train, att_neg_train, att_pos_train], f)

    with open('finetune/train_datas.pkl', 'rb') as f:
        train_datas = pickle.load(f)
   
    test_datas = copy.deepcopy(train_datas[:len(train_datas)//100])
    train_datas = train_datas[len(train_datas)//100:]
    ...
    with open('finetune/train_list_graphs.pkl', 'wb') as f:
        pickle.dump((train_list, train_graphs), f)
    with open('finetune/test_list_graphs.pkl', 'wb') as f:
        pickle.dump((test_list, test_graphs), f)
    ...
    for e in range(EPOCH):
        for i in tqdm(range(0, len(att_anchor_train), TRAIN_BATCH)):
            if i+TRAIN_BATCH > len(att_anchor_train):
                break
            batch_graph_anchor = att_anchor_train[i:i+TRAIN_BATCH]
            batch_graph_neg = att_neg_train[i:i+TRAIN_BATCH]
            batch_graph_pos = att_pos_train[i:i+TRAIN_BATCH]
        ...
        if e%5==0:
            ...
            torch.save(state,'finetune/model{}.pth'.format(e))
